...
accept_send(TALLOC_CTX *mem_ctx, struct tevent_context *ev, int listen_sock) {
  ...
  state->fde = tevent_add_fd(ev, state, listen_sock, TEVENT_FD_READ,
                             accept_handler, /*private data is `req` now*/ req);
...
static struct tevent_req *read_send(TALLOC_CTX *mem_ctx,
  ...
  state->fde = tevent_add_fd(ev, state, fd, TEVENT_FD_READ, read_handler, req);
  if (tevent_req_nomem(state->fde, req)) {
    return tevent_req_post(req, ev);
  }
  return req;
...
static struct tevent_req *write_send(TALLOC_CTX *mem_ctx,
  ...
  state->fde =
      tevent_add_fd(ev, state, fd, TEVENT_FD_WRITE, write_handler, req);
  if (tevent_req_nomem(state->fde, req)) {
    return tevent_req_post(req, ev);
  }
  return req;
...
static void writeall_done(struct tevent_req *subreq) {
  ...
  struct writeall_state *state = tevent_req_data(req, struct writeall_state);
  ssize_t nwritten;
  int err = 0;
...
static struct tevent_req *echo_send(TALLOC_CTX *mem_ctx,
  ...
  subreq = read_send(state, state->ev, state->fd, state->buf,
                     talloc_get_size(state->buf));
  if (tevent_req_nomem(subreq, req)) {
    return tevent_req_post(req, ev);
  }
...
static void echo_read_done(struct tevent_req *subreq) {
  struct tevent_req *req = tevent_req_callback_data(subreq, struct tevent_req);
  struct echo_state *state = tevent_req_data(req, struct echo_state);
  ssize_t nread;
  int err;
...
/**
 * 创建事件 `req` 和 `subreq`：
 * - 当 `subreq` 完成时，调用 `echo_server_accepted` 函数，`req` 作为
 * `private_data` 参数传递给该函数
 * - 在初始化`subreq`时， 其通过 `tevent_add_fd` 函数为 `listen_sock` 添加了一个
 * `TEVENT_FD_READ` 事件， 该事件的回调函数为 `accept_handler`，`subreq` 作为
 * `private_data` 参数传递给该函数，该回调函数 将把 `subreq` 的状态设置为
 * `TEVENT_REQ_DONE`。
 */
static struct tevent_req *echo_server_send(TALLOC_CTX *mem_ctx,
  ...
  /**
   * req的state被设置为PROGRESS，但此时这个req还没有被执行
   *
   * @code
   ...
   *
   * void *pdata = &state;
   * void *data = talloc_zero_size(req, sizeof(struct echo_server_state));
   * req->data = data;
   * void **ppdata = (void **)pdata;
   * *ppdata = data;
   * @endcode
   */
  req = tevent_req_create(mem_ctx, &state, struct echo_server_state);
  if (req == NULL) {
    return NULL;
  }
  state->ev = ev;
  ...
  /**
   * 当 `listen_sock` 可读时，将调用 `accept_handler` 函数，
   * 其会通过 `tevent_req_done` 函数将 `subreq` 的状态设置为 `TEVENT_REQ_DONE`
   */
  ...
  // call `echo_server_accepted` when `subreq` on completion, while passing
  // `req` as `private_data` argument
...
static void echo_server_accepted(struct tevent_req *subreq) {
  struct tevent_req *req = tevent_req_callback_data(subreq, struct tevent_req);
  struct echo_server_state *state =
      tevent_req_data(req, struct echo_server_state);
  int sock, err;
...
int main(int argc, const char **argv) {
  ...
  // 该函数仅初始化事件，而没有执行事件
  // 1. init `req`'s memory allocation:
  //    - create `state` as `req`'s talloc children
  // 2. create `subreq`:
  //    - `subreq->async.fn` = `echo_server_accepted`
